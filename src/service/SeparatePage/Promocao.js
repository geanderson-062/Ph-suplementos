const ProducsData = [
  {
    type: "Pré treino Psicho Killer Demons-Lab",
    description: "294g",
    priceWithoutDiscount: "100.00",
    discountPrice: "95,99",
    imageUrl:
      "https://d2vq4s943o8cb4.cloudfront.net/Custom/Content/Products/02/76/0276_creatina-hardcore-integralmedica-2312_m2_636668372702493590.png",
    link: "/",
  },

  {
    type: "Hipercalórico Diabo-Verde",
    description: "2,5Kg",
    priceWithoutDiscount: "90.00",
    discountPrice: "85,99",
    imageUrl:
      "https://d2vq4s943o8cb4.cloudfront.net/Custom/Content/Products/02/76/0276_creatina-hardcore-integralmedica-2312_m2_636668372702493590.png",
    link: "/",
  },

  {
    type: "Whey Protein Integral medica",
    description: "900g Refil",
    priceWithoutDiscount: "100.00",
    discountPrice: "95,99",
    imageUrl:
      "https://d2vq4s943o8cb4.cloudfront.net/Custom/Content/Products/02/76/0276_creatina-hardcore-integralmedica-2312_m2_636668372702493590.png",
    link: "/",
  },

  {
    type: "Creatina Integral medica",
    description: "150g",
    priceWithoutDiscount: "90.00$",
    discountPrice: "85,99$",
    imageUrl:
      "https://d2vq4s943o8cb4.cloudfront.net/Custom/Content/Products/02/76/0276_creatina-hardcore-integralmedica-2312_m2_636668372702493590.png",
    link: "/",
  },
];

export default ProducsData;
